...
// Subset of the eavesly_module_results_with_feedback view this page selects.
// NOT the alerts view: eavesly_alerts_with_feedback filters to alert_sent =
// true, which hides every clean walkthrough and made adoption read 0%. This
// view carries one row per evaluation regardless of alert status. The view is
// in the generated Database types, so no `as any` client cast is needed.
type GotaViewRow = {
  call_id: string | null
...
export async function fetchGotaEvaluations(
  ...
  const rows = await fetchAllPaginated<GotaViewRow>((from, to) => {
    let q = supabase
      .from('eavesly_module_results_with_feedback')
      .select(
        'call_id, agent_email, contact_name, created_at, has_violation, is_reviewed, accurate, result_json',
      )
      .eq('module_name', GOTA_MODULE)
      .gte('created_at', startOfBusinessDay(startDate).toISOString())
      .lte('created_at', endOfBusinessDay(endDate).toISOString())
  ...
  return rows
    .filter(row => row.call_id !== null && row.created_at !== null)
    .map(row => ({
      call_id: row.call_id!,
