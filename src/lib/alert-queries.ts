...
// SAFETY: generated types lag the alert view/tables. Values leaving this client
// are narrowed at their feature boundary rather than weakening the whole module.
...
export async function fetchUserScope(email: string): Promise<UserScope> {
  ...
  return {
    ...
    managedAgents: Array.isArray(agentsRes.data)
      ? agentsRes.data.flatMap((agent: unknown) => {
        const email = stringAt(agent, 'agent_email')
        return email ? [email] : []
      })
      : [],
  }
...
export async function fetchAlerts(
  ...
  const rows = await fetchAllPaginated<AlertWithFeedback & {
    ...
    // SAFETY: the aliased view projection above is represented by this local row type;
    // consumers still narrow projected JSON scalars before use.
    return q as unknown as PromiseLike<{
      data: (AlertWithFeedback & {
        reason_escalation?: unknown
        reason_direct?: unknown
        reason_warm_transfer?: unknown
      })[] | null
...
export async function setAlertAck(input: {
  ...
  if (input.acked) {
    ...
    if (error && stringAt(error, 'code') !== '23505') {
...
export function extractEvidence(violationType: string, result: unknown): string {
  switch (violationType) {
    case 'manager_escalation': {
      const areas = record(record(result)?.call_overview)?.manager_focus_areas
      return Array.isArray(areas)
        ? areas.flatMap(area => {
          const quote = stringAt(area, 'quote')
          return quote ? [quote] : []
        }).join('; ')
        : ''
    }
    ...
    case 'gota_check':
      return stringAt(result, 'key_evidence_quote')
    case 'warm_transfer':
      return stringAt(result, 'warm_transfer_compliance', 'violation_reason')
    default:
      return ''
  }
}
...
export function extractReason(violationType: string, result: unknown): string {
  switch (violationType) {
    case 'manager_escalation':
      return stringAt(result, 'call_overview', 'manager_review_reason')
    case 'budget_compliance':
    case 'litigation_check':
    case 'program_expectations':
    case 'gota_check':
      return stringAt(result, 'violation_reason')
    case 'warm_transfer':
      return stringAt(result, 'warm_transfer_compliance', 'violation_reason')
    default:
      return ''
