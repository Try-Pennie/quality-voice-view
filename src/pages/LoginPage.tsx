...
import StagingPasswordLogin from '../components/StagingPasswordLogin'
...
export default function LoginPage() {
  ...
  useEffect(() => {
    if (user && !loading) {
      if (isStaging) {
        // A new page clears account-scoped query caches when testers switch identities.
        const destination = user.email === 'eavesly-staging-kris-xuvveqaiz@trypennie.com'
          ? '/dashboard/alerts/DEMO-APPROVAL-001/full_qa?range=outstanding&status=awaiting_approval'
          : '/dashboard/alerts/REAL-REVIEW-021/full_qa?range=outstanding&status=awaiting_manager&search=Production%20sample'
        ...
        <div className="w-full max-w-sm space-y-8 animate-pennie-rise">
          ...
          <div className="space-y-2">
            ...
            <h2 className="font-display text-3xl tracking-[-0.02em] text-pennie-navy font-bold">
              {isStaging ? 'Staging access.' : 'Welcome back.'}
            </h2>
            <p className="text-pennie-graphite/70 text-sm">
              {isStaging
                ? 'Choose a view and enter the preview password Noah shared with you.'
                : 'Sign in with your Pennie Google account to view alerts, calls, and team performance.'}
            </p>
          </div>

          {isStaging ? (
            <StagingPasswordLogin />
          ) : (
            <button
              type="button"
          ...
          <p className="text-xs text-muted-foreground text-center">
            {isStaging ? (
              'Private preview · Contains real customer information. Please keep the password and recordings private.'
            ) : (
              <>
