-- Add total_alerts_count to the agent daily metrics MV and expose it through
-- team_daily_metrics / agent_daily_metrics so the leaderboard can show
-- workload alongside open alerts. open_alerts already filters on
-- has_violation; total counts every fired alert regardless of outcome.
...
-- RPCs need a DROP+CREATE since CREATE OR REPLACE can't change the return shape.
