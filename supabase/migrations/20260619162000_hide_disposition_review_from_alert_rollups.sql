-- Temporarily hide disposition-review production-test alerts from manager-facing
-- team/agent alert rollups. The raw module results stay in Supabase; only the
-- materialized metrics consumed by the frontend exclude this alert type.
...
-- Rebuild existing rollup rows immediately with disposition-review excluded.
-- The cron job (private.refresh_agent_daily_metrics) keeps it fresh thereafter.
