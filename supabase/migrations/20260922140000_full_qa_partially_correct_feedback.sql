-- Mixed criterion feedback is neither a confirmed score nor a replacement score.
-- It requires a brief explanation and a JSON-null corrected_value.
  ...
  if (select count(*) from jsonb_array_elements(p_corrections)) <> (select count(distinct c->>'criterion_key') from jsonb_array_elements(p_corrections)c)
    or exists (
      ...
      where m is null or jsonb_typeof(c)<>'object' or (select count(*) from jsonb_object_keys(c))<>4
        ...
        or c->>'disposition' not in ('confirmed','corrected','partially_correct','needs_context')
