...
insert into public.achieve_client_sfdc_map(sfdc_lead_id, client_id) values
  ('LEAD-A', 'CLIENT-A'), ('LEAD-A2', 'CLIENT-A'),
  ('LEAD-B', 'CLIENT-B'),
...
begin
  ...
  if overview->'scope'->>'total_submissions' <> '8'
    or overview->'ratings' <> '{"fair":1,"good":3,"other":1,"poor":3}'::jsonb then
    raise exception 'Form denominator changed: %', overview;
  end if;
  if overview->'coverage' <> '{"unresolved":1,"call_associated":7,"agent_unavailable":2,"exact_agent_attributed":5}'::jsonb then
  ...
  if overview->'qa'->'alignment' <> '{"ai_only":1,"both_clear":1,"human_only":1,"both_concern":1,"overlap_calls":4}'::jsonb then
  ...
  if not exists (
    ...
    where row->>'achieve_agent_email' = 'rep-a@example.test'
      and row->>'total_submissions' = '3'
      and row->>'ai_total' = '2'
      and row->>'ai_pass' = '2'
      and row->>'overlap_calls' = '2'
      and row->>'both_clear' = '1'
  ...
  if representative_detail->'coverage'->>'total' <> '3'
  ...
  if representative_detail->'rows'->2->>'notes' <> 'Clear handoff.' then
