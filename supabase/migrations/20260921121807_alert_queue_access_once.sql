-- Compute the existing caller visibility check once per alert, not again for
-- each masked field and Outstanding filter. Preserve the ordered feedback join.
-- OFFSET 0 changes evaluation frequency, not the permission rule or row set.
BEGIN;
