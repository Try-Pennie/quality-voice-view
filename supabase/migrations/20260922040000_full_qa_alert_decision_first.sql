-- Alert decisions do not require a full scorecard, findings, or completed coaching.
-- Sparse corrections contain only explicit manager answers, never implied confirmations.
-- Existing immutable reviews and the scoped/revision-guarded public RPC are unchanged.
set local lock_timeout = '3s';
set local statement_timeout = '10s';

alter table public.eavesly_full_qa_review_revisions
  drop constraint eavesly_full_qa_review_revisions_corrections_check;
alter table public.eavesly_full_qa_review_revisions
  add constraint eavesly_full_qa_review_revisions_corrections_check
  check (jsonb_typeof(corrections) = 'array' and jsonb_array_length(corrections) <= 23) not valid;
alter table public.eavesly_full_qa_review_revisions
  validate constraint eavesly_full_qa_review_revisions_corrections_check;
...
begin
  if p_source is null or p_manifest is null or p_corrections is null or p_findings is null or p_escalation is null
    ...
    or jsonb_typeof(p_findings) is distinct from 'array' or p_reason is null
    or char_length(private.trim_internal_review_text(p_reason))>4000
    or (not p_escalation and (v_reason is null or char_length(v_reason)<12)) then
    return false;
  end if;
    ...
    or exists (
      ...
      where m is null or jsonb_typeof(c)<>'object' or (select count(*) from jsonb_object_keys(c))<>4
        ...
        or case c->>'disposition'
          when 'confirmed' then c->'corrected_value' is distinct from (p_source #> string_to_array(m->>'score_path','.'))
            or not ((m->'domain') @> jsonb_build_array(c->'corrected_value')) or c->'reason'<>'null'::jsonb
  ...
  if p_inaccuracy_reason is not null and (p_escalation or p_inaccuracy_reason not in (
  ...
  if (p_action is null and v_action_details is not null)
    or (p_action is not null and (p_action not in ('coached','escalated','follow_up_later','no_action_needed')
