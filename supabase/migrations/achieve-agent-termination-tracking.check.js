// Static guard for termination seeds, service-only access, and enrollment-date monitoring.
...
const activity = await readFile(new URL('./20260830120000_achieve_termination_enrollment_activity.sql', import.meta.url), 'utf8')
...
for (const required of [
  ...
  "bucket.enrollment_date > (termination.terminated_at at time zone 'America/New_York')::date",
  ">= (p_end_at at time zone 'America/New_York')::date - 30",
  "'latest_post_term_enrollment_on'",
  "'enrollments_post_termination'",
  'revoke execute on function public.ingest_achieve_termination_enrollment_activity',
  'grant execute on function public.list_achieve_agent_termination_monitoring(timestamptz)',
]) {
  assert.ok(activity.includes(required), `missing termination enrollment invariant: ${required}`)
}
assert.ok(!activity.includes('welcome_call_agent_log'))
assert.ok(!activity.includes('first_seen_on'))
assert.ok(!activity.includes('last_seen_on'))
