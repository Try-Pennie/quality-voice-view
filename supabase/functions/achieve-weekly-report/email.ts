...
function emailBody(report: AchieveManagementReport, portalUrl: string): { readonly text: string; readonly html: string } {
  ...
  const terminationRows = report.terminations.map(termination => `<tr><td style="${LEFT_CELL}">${html(termination.agentName)}</td><td style="${CELL}">${html(termination.terminatedAt.slice(0, 10))}</td><td style="${CELL}">${termination.latestPostTermEnrollmentOn ?? '—'}</td><td style="${CELL};font-weight:600">${termination.enrollmentsPostTermination}</td></tr>`).join('')
