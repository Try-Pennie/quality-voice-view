...
function emailBody(
  ...
  const rows = representatives.map(representative => {
    ...
    return `<tr>
      <td style="min-width:220px;padding:14px;border-bottom:1px solid #e2e8f0"><strong style="font-size:15px">${html(representative.agentName)}</strong>${newEmails.has(representative.agentEmail) ? ' <span style="display:inline-block;padding:3px 7px;border-radius:999px;background:#dcfce7;color:#166534;font-size:10px;font-weight:700">New</span>' : ''}<br><span style="color:#64748b;font-size:12px">${html(representative.agentEmail)}</span><br><span style="color:#64748b;font-size:11px">${latest}</span><br><span style="display:inline-block;margin-top:8px;padding:5px 9px;border-radius:999px;background:#fef3c7;color:#92400e;font-size:11px;font-weight:700">Persistent high risk${periodRanks ? ` · 2w #${periodRanks[2]} · 4w #${periodRanks[4]} · 6w #${periodRanks[6]}` : ''}</span></td>
  ...
  const table = rows === '' ? '' : `<div style="margin:20px 0;overflow-x:auto;border:1px solid #e2e8f0;border-radius:12px"><table style="width:100%;min-width:1420px;border-collapse:collapse;background:#fff;font-size:13px">
