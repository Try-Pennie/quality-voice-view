...
- Real decisions require an action plus distinct 12–4000 character violation/action details. False alarms require a reason plus a 12–4000 character explanation. `disposition_review` and `achieve_welcome_call_qa` retain their existing direct-writer behavior.
...
- The alert view keeps its existing columns in order and appends scoped structured-review/current-decision fields. A negative `current_decision_id` denotes a revision-1 legacy super-admin acknowledgment; ordinary acknowledgments never approve.
