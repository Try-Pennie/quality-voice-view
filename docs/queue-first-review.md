...
The Review route now leads with the selected ET period, role-aware queue selector, search, and **Review next**. Manager first reviews remain the default for managers; **Awaiting Kris’s approval** remains the default for god-mode users. Existing `status`, date, sort, workload, manager, outcome, module, search, and drawer URL values remain unchanged.
...
These are real Chromium renders with synthetic `@example.test` fixtures, not live production data. Before/after queue captures use the same scenario, ET dates, and viewport: desktop 1440×900, mobile 375×844. Before code is identical to base `a33be83306da0709ed6c3e4405f50e023ab90c40`; after code is `d6e00dc27ee0333bc08e579b90e4953ef11cba74` (refreshed for the “Kris’s approval” wording). The expanded workload example uses a separate six-alert fixture at 1280px width.

| Screen | Before | After |
| --- | --- | --- |
| Kris’s queue | [Before](screenshots/queue-first-9cd1c96/before-director-queue.png) | [After](screenshots/queue-first-9cd1c96/after-director-queue.png) |
...
| Kris’s approval | — | [Manager explanation, evidence, fixed actions](screenshots/queue-first-9cd1c96/after-director-approval.png) |
...
## Approval-label update at `d6e00dc`

The queue, headline, row status, manager workload column, drawer, and status help now say **Awaiting Kris’s approval**. This names the intended business reviewer without changing approval permissions or replacing recorded decision authors. Seven focused browser/logic checks, application/test TypeScript, and the production build passed; screenshots were refreshed from those checks.

## Original phase verification at `9cd1c96`
