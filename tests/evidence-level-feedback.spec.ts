...
test('passage judgments stay independent, optional, source-bound, and survive save failures', async ({ page, browser }, testInfo) => {
  ...
  await expect(consent.locator('summary').filter({ hasText: 'Optional criterion score adjustment' })).toHaveCount(0)
  ...
  await expect(general).toContainText('Saved call-level review focus.')
  ...
  await consentEvidence.getByRole('button', { name: /^Find in transcript/ }).click()
  ...
  await expect(consentEvidence).toHaveAttribute('aria-current', 'location')
  await expect(representationEvidence).not.toHaveAttribute('aria-current', 'location')
  await transcript.getByRole('button', { name: 'Back to evidence', exact: true }).click()
  await expect(consentEvidence).toBeFocused()
  await representationEvidence.getByRole('button', { name: /^Find in transcript/ }).click()
  await expect(transcript.getByRole('complementary', { name: 'Selected evidence', exact: true })).toContainText('Accurate representations')
  await expect(representationEvidence).toHaveAttribute('aria-current', 'location')
  await expect(consentEvidence).not.toHaveAttribute('aria-current', 'location')
  await transcript.getByRole('button', { name: 'Back to evidence', exact: true }).click()
  ...
  await expect(consentEvidence.getByRole('textbox')).toHaveCount(0)
  await expect(consentEvidence.getByRole('button', { name: 'Add comment', exact: true })).toBeVisible()
  await representationEvidence.getByRole('radio', { name: 'Evidence: Incorrect', exact: true }).check()
  await representationEvidence.getByRole('button', { name: 'Add comment', exact: true }).click()
  ...
  await generalEvidence.getByRole('button', { name: 'Add comment', exact: true }).click()
  ...
  await expect(representationEvidence.getByText('x', { exact: true })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Update review', exact: true })).toBeDisabled()
  await representationEvidence.getByRole('button', { name: 'Edit comment', exact: true }).click()
  await expect(representationEvidence.getByRole('textbox', { name: 'Accurate representations evidence comment', exact: true })).toHaveValue('x')
  await representationEvidence.getByRole('button', { name: 'Done', exact: true }).click()
  await expect(representationEvidence.getByRole('button', { name: 'Edit comment', exact: true })).toBeFocused()
  await expect(page.getByRole('button', { name: 'Update review', exact: true })).toBeDisabled()
  await representationEvidence.getByRole('button', { name: 'Edit comment', exact: true }).click()
