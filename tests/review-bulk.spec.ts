...
test('bulk approval bounds database writes and keeps failed reviews in the queue', async ({ page }) => {
  ...
  let release = () => {}
  state.ackGate = new Promise<void>(resolve => { release = resolve })
  state.failedAckIds.add('approve-11')
  ...
  await expect.poll(() => state.maxAckInFlight).toBe(10)
  release()
  ...
  expect(state.maxAckInFlight).toBe(10)
  expect(state.rows.filter(row => row.acker_emails.length > 0)).toHaveLength(11)
