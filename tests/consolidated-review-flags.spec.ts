...
test('one flag list pairs saved failures with passages without losing contexts or inventing score responses', async ({ page }, testInfo) => {
  ...
  await expect(flags.locator('blockquote:visible')).toHaveCount(3)
  const credit = flags.getByRole('article', { name: 'Credit pull consent', exact: true })
  await expect(credit).toContainText('No credit pull consent')
  for (const context of ['Saved criterion context.', 'Additional call-level context for consent.', 'Additional red-flag context for consent.']) {
    await expect(credit).toContainText(context)
  }
  ...
  await expect(outcome).toContainText('Additional call-level context for the guarantee.')
  await expect(outcome.getByRole('radio')).toHaveCount(0)
  const additional = flags.getByRole('article', { name: 'Additional flagged passage', exact: true })
  await expect(additional.locator('blockquote')).toHaveText(unlinked.quote)
  await expect(additional).toContainText('without linking it to a specific failed rule')
  await expect(additional.getByRole('radio')).toHaveCount(0)
...
test('a critical label stays on its exact supporting quote and duplicate passage contexts keep their source', async ({ page }) => {
  ...
  const flags = page.getByRole('region', { name: 'What Eavesly flagged', exact: true })
  const credit = flags.getByRole('article', { name: 'Credit pull consent', exact: true })
  await expect(credit.locator('figure')).toHaveCount(2)
  await expect(credit.locator('figure').filter({ has: page.locator('blockquote').filter({ hasText: QUOTES[0] }) })).not.toContainText('Outcome guarantee')
  await expect(credit.locator('figure').filter({ has: page.locator('blockquote').filter({ hasText: QUOTES[1] }) })).toContainText('Flag: Outcome guarantee')
  await expect(credit).toContainText('Call-level context (handling agent): Another call-level context for the second quote.')
  await expect(credit).toContainText('Flag context (Outcome guarantee) (handling agent): Flag-specific context for the second quote.')
  await expect(credit).toContainText('Saved context (handling agent): Another criterion context for the second quote.')
  await expect(flags.getByRole('article', { name: 'Additional flagged passage', exact: true })).toHaveCount(0)
})
...
test('same words from a different speaker stay unlinked and a flag without an excerpt stays honest', async ({ page }) => {
  ...
  await expect(flags.getByRole('article', { name: 'Additional flagged passage', exact: true })).toContainText('Customer statement, not agent evidence.')
  const guarantee = flags.getByRole('article', { name: 'Outcome guarantee', exact: true })
  await expect(guarantee).toContainText('No readable excerpt was saved')
