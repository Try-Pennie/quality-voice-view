...
test('coaching belongs to Yes; hidden drafts never block No and survive toggles and failed saves', async ({ page }, testInfo) => {
  ...
  await expect(page.getByText('Coaching issues won’t be included when you save No.', { exact: false })).toBeVisible()
  await expect(save).toBeEnabled()
  await yes.check()
  ...
  for (const width of [1280, 375]) {
    ...
    await decision.evaluate(element => element.scrollIntoView({ block: 'start' }))
    await expect(coaching.getByRole('heading', { name: 'Coaching issues (optional)', exact: true })).toBeVisible()
