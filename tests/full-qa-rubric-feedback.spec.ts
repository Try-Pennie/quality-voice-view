...
const criterionArticle = (page: Page, criterion: string) => page.getByRole('article', { name: criterion, exact: true })
const criterionDisclosure = (page: Page, criterion: string) => criterionArticle(page, criterion).locator('summary').filter({ hasText: 'Optional criterion score adjustment' })
const visibleCriterionDisclosures = (page: Page) => page.getByRole('region', { name: 'What Eavesly flagged', exact: true })
  .locator('article:visible').locator('summary').filter({ hasText: 'Optional criterion score adjustment' })
async function openCriterionAdjustment(page: Page, criterion: string) {
  if (await criterionDisclosure(page, criterion).count() === 0) await page.getByRole('button', { name: /View full scorecard/ }).click()
  await criterionDisclosure(page, criterion).click()
  await expect(response(page, criterion)).toBeVisible()
}
...
test('neutral guidance links to the incomplete decision, score or coaching section without changing the draft', async ({ page }) => {
  ...
  const consent = criterionArticle(page, 'Credit pull consent')
  await openCriterionAdjustment(page, 'Credit pull consent')
...
test('Full QA saves string-scale corrections and optional coaching for a warranted alert', async ({ page }, testInfo) => {
  ...
  await expect(responseColumn).toHaveCount(1)
  await expect(criterionDisclosure(page, 'Credit pull consent')).toHaveCount(0)
  await page.setViewportSize({ width: 1280, height: 720 })

  await expect(page.getByRole('form', { name: 'Full QA rubric review' })).toBeVisible()
  await expect(page.getByRole('heading', { name: 'What Eavesly flagged', exact: true })).toBeVisible()
  ...
  await expect(visibleCriterionDisclosures(page)).toHaveCount(0)
  ...
  await expect(consent.getByText('Evidence tied to this claim', { exact: true })).toHaveCount(1)
  ...
  await expect(visibleCriterionDisclosures(page)).toHaveCount(23)
  await openCriterionAdjustment(page, 'Call recording disclosure')
  await openCriterionAdjustment(page, 'Credit pull consent')
  await openCriterionAdjustment(page, 'professional tone')

  await expect(page.getByRole('radio', { name: 'Need more context', exact: true, includeHidden: true })).toHaveCount(0)
  await response(page, 'Call recording disclosure').getByRole('radio', { name: 'Incorrect', exact: true }).check()
  await page.getByRole('textbox', { name: 'Call recording disclosure correction reason' }).fill(correctionReason)
  await response(page, 'Credit pull consent').getByRole('radio', { name: 'Incorrect', exact: true }).check()
  ...
  await expect(response(page, 'Social security verification')).toHaveCount(0)
  await expect(saveButton(page)).toBeDisabled()
  ...
  expect((write as { p_corrections: unknown[] }).p_corrections).toHaveLength(3)
  expect((write as { p_corrections: unknown[] }).p_corrections).toEqual(expect.arrayContaining([
    { criterion_key: 'call_recording_disclosure', disposition: 'corrected', corrected_value: 'fail', reason: correctionReason },
    { criterion_key: 'credit_pull_consent', disposition: 'corrected', corrected_value: 'pass', reason: correctionReason },
  ...
  await expect(page.getByText('Changed to: Does not meet the rule', { exact: true })).toBeVisible()
  ...
  await expect(outcome).toContainText('Saved criterion-level opinions (3)')
  ...
  await expect(outcome).toContainText('Call recording disclosure: Eavesly said Meets the rule → manager changed to Does not meet the rule — ' + correctionReason)
  ...
  await expect(adminPage.getByRole('region', { name: 'What Eavesly flagged', exact: true })).toBeVisible()
  await expect(adminPage.getByText('Changed to: Does not meet the rule', { exact: true })).toBeVisible()
...
test('saved needs-context responses remain visible without offering new ones', async ({ browser }) => {
  const scores: Readonly<Record<string, unknown>> = { ...FULL_QA_RESULT.compliance_scorecard, ...FULL_QA_RESULT.customer_experience_scorecard,
    ...FULL_QA_RESULT.sales_process_scorecard, ...FULL_QA_RESULT.program_expectations_scorecard }
  const corrections = FULL_QA_CRITERIA.map(criterion => criterion.key === 'credit_pull_consent'
    ? { criterion_key: criterion.key, disposition: 'needs_context', corrected_value: null, reason: contextReason }
    : { criterion_key: criterion.key, disposition: 'confirmed', corrected_value: scores[criterion.key], reason: null })
  const row = alertRow('saved-context', { is_reviewed: true, feedback_id: 42, feedback_by: EMAIL, accurate: false, reviewed_at: '2026-09-05T14:00:00Z' })
  const fullQaReviews = new Map([['saved-context', { feedback_revision: 1, corrections, findings: [],
    escalation_justified: false, escalation_reason: escalationReason, escalation_inaccuracy_reason: 'wrong_context',
    action_taken: null, action_details: null, saved_by: EMAIL, saved_at: '2026-09-05T14:00:00Z' }]])
  const manager = await browser.newPage()
  const state = await reviewFixture(manager, [structuredClone(row)], { fullQaReviews })
  await manager.goto('/dashboard/alerts/saved-context/full_qa')
  const consent = manager.getByRole('article', { name: 'Credit pull consent', exact: true })
  await expect(consent.getByText('Needs more context', { exact: true })).toBeVisible()
  await expect(consent.getByText(contextReason, { exact: true })).toBeVisible()
  await expect(consent.getByRole('radio')).toHaveCount(3)
  await expect(consent.getByRole('radio', { checked: true })).toHaveCount(0)
  await expect(consent.getByRole('textbox')).toHaveCount(0)
  await manager.getByRole('textbox', { name: 'Explain your decision' }).fill('Updated alert judgment without altering the historical unresolved response.')
  await saveButton(manager).click()
  await expect(manager.getByText('Full QA review saved')).toBeVisible()
  expect(state.writes).toEqual(expect.arrayContaining([expect.objectContaining({ p_corrections: corrections })]))
  await manager.close()

  const kris = await browser.newPage()
  await reviewFixture(kris, [structuredClone(row)], { fullQaReviews, god: true, email: 'director@example.test' })
  await kris.goto('/dashboard/alerts/saved-context/full_qa')
  await expect(kris.getByRole('region', { name: 'Manager’s review', exact: true })).toContainText('manager needs more context — ' + contextReason)
  await expect(kris.getByRole('region', { name: 'What Eavesly flagged', exact: true })).toBeVisible()
  await expect(kris.getByText('Needs more context', { exact: true })).toBeVisible()
  await expect(kris.getByRole('radio')).toHaveCount(0)
  await kris.close()
})
...
test('score answers and an explicit verdict save without manufacturing any coaching issue', async ({ page }) => {
  ...
  await response(page, 'Credit pull consent').getByRole('radio', { name: 'Incorrect', exact: true }).check()
  await page.getByRole('textbox', { name: 'Credit pull consent correction reason' }).fill(correctionReason)
  await openCriterionAdjustment(page, 'Accurate representations')
...
test('a warranted alert with two distinct issues records shared and repeated criterion links while one pending save holds every submit path', async ({ page }) => {
  ...
  await page.getByRole('radio', { name: 'Yes, the alert was warranted', exact: true }).check()
  const consent = criterionArticle(page, 'Credit pull consent')
  const representations = criterionArticle(page, 'Accurate representations')
  await openCriterionAdjustment(page, 'Credit pull consent')
  await consent.getByRole('button', { name: 'Add as coaching issue' }).click()
  ...
  await expect(response(page, 'Credit pull consent').getByRole('radio', { name: 'Incorrect', exact: true })).toBeDisabled()
...
test('a realistic supported seed keeps the reason, first evidence, and first decision on the first screen', async ({ page }) => {
  ...
  const consent = page.getByRole('article', { name: 'Credit pull consent', exact: true })
  await expectCenteredDesktopDialog(page)
  const reasonDetails = summary.locator('details').first()
  await expect(reasonDetails.locator('summary')).toContainText(reason)
  await expect(reasonDetails.locator('summary')).toContainText('Show full saved reason')
  await expect(consent.locator('blockquote').first()).toBeVisible()
  await expectFullscreenMobileDialog(page)
  await page.getByRole('button', { name: 'Review', exact: true }).click()
  await reasonDetails.locator('summary').click()
  await expect(reasonDetails.locator('p').getByText(reason, { exact: true })).toBeVisible()
  await expect(saveButton(page)).toBeInViewport()
  ...
  const representationEvidence = criterionArticle(page, 'Accurate representations')
    .getByRole('group', { name: 'Accurate representations evidence 1', exact: true })
  await expect(representationEvidence.getByText('You will be debt-free in 48 months, guaranteed.', { exact: true })).toBeVisible()
  // The base fixture retains four program gaps, one CX concern and one process gap alongside two compliance concerns.
  await expect(page.getByRole('region', { name: 'What Eavesly flagged', exact: true }).getByRole('article', { name: 'Credit pull consent', exact: true })).toBeVisible()
...
test('focused scorecard respects categorical concerns and enrollment gating, while toggling preserves edits', async ({ page }, testInfo) => {
  ...
  for (const label of ['Credit pull consent', 'Accurate representations', 'professional tone', 'patience empathy', 'step2 credit review', 'step6 debt resolution']) {
    await expect(criterionArticle(page, label)).toBeVisible()
  }
  for (const label of ['Call recording disclosure', 'active listening', 'step1 agenda setting', 'step4 paydown projections', 'phase recovery covered']) {
    await expect(criterionArticle(page, label)).toBeHidden()
  }
  ...
  await expect(response(page, 'Credit pull consent').getByRole('radio', { name: 'Incorrect', exact: true })).toBeChecked()
  await page.getByRole('textbox', { name: 'Credit pull consent correction reason' }).fill(correctionReason)
  await openCriterionAdjustment(page, 'phase recovery covered')
  await response(page, 'phase recovery covered').getByRole('radio', { name: 'Incorrect', exact: true }).check()
  await page.getByRole('textbox', { name: 'phase recovery covered correction reason' }).fill(correctionReason)
  await page.getByRole('button', { name: 'Show only items to check' }).click()
  await expect(visibleCriterionDisclosures(page)).toHaveCount(2)
  await expect(criterionArticle(page, 'phase recovery covered')).toBeVisible()
  await expect(page.getByRole('textbox', { name: 'phase recovery covered correction reason' })).toHaveValue(correctionReason)
  const expandAgain = page.getByRole('button', { name: 'View full scorecard · 23 criteria' })
  for (const width of [320, 375, 414, 768]) {
    await page.setViewportSize({ width, height: 844 })
    if (width === 320) await page.getByRole('button', { name: 'Review', exact: true }).click()
    await expandAgain.scrollIntoViewIfNeeded()
    expect(await page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
...
test('evidence shows readable quotes with context, keeps notes distinct, and preserves unfamiliar data under details', async ({ page }) => {
  ...
  await expect(consent.getByText('No reason saved for this score.', { exact: true })).toHaveCount(0)
  const malformed = consent.getByRole('group', { name: 'Credit pull consent evidence 2', exact: true })
  await expect(malformed).toContainText('No readable evidence was saved for this claim.')
  await expect(malformed.getByRole('radio')).toHaveCount(0)
  await expect(consent.locator('pre')).toBeHidden()
  await consent.getByText('Rule and saved evidence', { exact: true }).click()
  await expect(consent.locator('pre')).toContainText('Unfamiliar evidence must remain available.')
  const note = criterionArticle(page, 'Accurate representations')
  const whyFlagged = note.getByText('Why this was flagged', { exact: true }).locator('..')
  await expect(whyFlagged).toBeVisible()
  await expect(whyFlagged.getByText('The model describes an unqualified outcome promise.', { exact: true })).toBeVisible()
  const noteOccurrence = note.getByRole('group', { name: 'Accurate representations evidence 1', exact: true })
  await expect(noteOccurrence.getByText('The model describes an unqualified outcome promise.', { exact: true })).toBeVisible()
  await expect(noteOccurrence).toContainText('Saved note — not a transcript quote')
  ...
  await expect(empty.getByText('No readable evidence was saved for this claim. Check the transcript before judging it.', { exact: true })).toBeVisible()
})
...
test('empty focus is not a cleared alert and correcting an unavailable score is optional but must be explicit', async ({ page }) => {
  ...
  await expect(visibleCriterionDisclosures(page)).toHaveCount(0)
  await expect(criterionArticle(page, 'Social security verification')).toBeVisible()
  await openCriterionAdjustment(page, 'Social security verification')
  const assessment = response(page, 'Social security verification')
  await expect(assessment.getByRole('radio')).toHaveCount(3)
  await expect(assessment.getByRole('radio', { checked: true })).toHaveCount(0)
  await expect(assessment.getByRole('radio', { name: 'Correct', exact: true })).toBeDisabled()
  await expect(page.getByText('No score was saved. You can still review the alert; correcting this score is optional.')).toBeVisible()
  await expect(page.getByRole('textbox', { name: 'Social security verification correction reason' })).toHaveCount(0)
  await page.getByRole('radio', { name: 'No, the alert was unnecessary', exact: true }).check()
  await page.getByRole('textbox', { name: 'Explain your decision' }).fill(escalationReason)
  await page.getByText('Reason category (optional)', { exact: true }).click()
  ...
  await page.getByRole('textbox', { name: 'Social security verification correction reason' }).fill(correctionReason)
  await expect(saveButton(page)).toBeDisabled()
  await result.selectOption('not_applicable')
  await expect(saveButton(page)).toBeEnabled()
  await saveButton(page).click()
  await expect(page.getByText('Full QA review saved')).toBeVisible()
})
...
test('Agent profile reconciles approved findings and keeps pending, needs-context, and legacy rows uncounted', async ({ page }, testInfo) => {
  ...
  await expect(panel).toContainText('After approved coaching linked to this category (review-date proxy)')
...
test('saved alert reasons and criterion context explain concerns without turning coaching scores into triggers', async ({ page }) => {
  ...
  const consent = criterionArticle(page, 'Credit pull consent')
  await expect(consent.getByText('Eavesly flagged this', { exact: true })).toBeVisible()
  const firstEvidence = consent.getByRole('group', { name: 'Credit pull consent evidence 1', exact: true })
  await expect(firstEvidence.getByText(`Saved context: ${note}`, { exact: true })).toHaveCount(1)
  await expect(consent.getByText('Evidence tied to this claim', { exact: true })).toBeVisible()
  await expect(consent.getByText('No reason saved for this score.', { exact: true })).toHaveCount(0)
  ...
  await openCriterionAdjustment(page, 'Credit pull consent')
  await expect(consent.getByText('Criterion-level opinion (optional; separate from passage feedback)', { exact: true })).toBeVisible()
  ...
  await expect(coaching.getByText('Eavesly score concern', { exact: true })).toBeVisible()
  await expect(coaching.getByText('Eavesly flagged this', { exact: true })).toHaveCount(0)
  await expect(coaching.getByText('No reason saved for this score.', { exact: true })).toHaveCount(0)
})

test('a maximum observed saved reason stays available behind the compact overview', async ({ page }) => {
  ...
  const summary = page.getByRole('region', { name: 'Why Eavesly requested review', exact: true })
  const details = summary.locator('details').first()
  await expect(details.locator('summary')).toContainText(reason)
  await expect(details.locator('summary')).toContainText('Show full saved reason')
  await details.locator('summary').click()
  await expect(details.locator('p').getByText(reason, { exact: true })).toBeVisible()
})
...
test('missing or malformed explanations never become invented reasons or confirmed findings', async ({ page }) => {
  ...
  const summary = page.getByRole('region', { name: 'Why Eavesly requested review', exact: true })
  await expect(summary.getByText('No alert reason saved.', { exact: true })).toHaveCount(0)
  ...
  const evidence = consent.getByRole('group', { name: 'Credit pull consent evidence 1', exact: true })
  await expect(evidence.getByRole('radio', { name: 'Evidence: Correct', exact: true })).not.toBeChecked()
  await openCriterionAdjustment(page, 'Credit pull consent')
  await expect(response(page, 'Credit pull consent').getByRole('radio', { name: 'Correct', exact: true })).not.toBeChecked()
  await expect(page.getByRole('region', { name: 'Coaching issues', exact: true })).toHaveCount(0)
  await page.getByRole('radio', { name: 'Yes, the alert was warranted', exact: true }).check()
  await expect(page.getByText('No coaching issues added.', { exact: true })).toBeVisible()
...
test('program gaps remain section-level notes rather than invented per-criterion reasons', async ({ page }) => {
  ...
  await expect(criterion.getByText(/see the saved section notes above/)).toHaveCount(0)
