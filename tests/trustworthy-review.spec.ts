...
test('god-mode internal manager counts reconcile and filter the same inbox', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: 'Filter Manager Alpha Real alerts 1' })).toBeVisible()
...
test('god-mode reviewer summary credits latest decisions to feedback authors and filters matching cohort rows', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('button', { name: 'Filter reviewer-one Real alerts 1' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Filter reviewer-one False alarm 1' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Filter reviewer-one False rate 50%' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Filter reviewer-two Reviewed 2' })).toBeVisible()
  await expect(page.getByRole('button', { name: 'Filter reviewer-two False rate 100%' })).toBeVisible()
  ...
  await page.getByText('Team workload', { exact: true }).click()
  await page.getByRole('button', { name: 'Filter reviewer-one False rate 50%' }).click()
...
test('all-time outstanding loads every actionable state without date clipping or Team rollup reads', async ({ page }, testInfo) => {
  ...
  await expect(page.getByRole('button', { name: 'Filter all-time Corrections requested 1' })).toBeVisible()
